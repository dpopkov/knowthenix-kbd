...
class Ex8Exceptions {
    ...
    fun handler2() {
        val scope = CoroutineScope(Dispatchers.Default + handler("top"))
        scope.launch(CoroutineName("Cor-1")) {
            log("in 1st launch")
            launch(handler("child") + CoroutineName("Cor-1.1")) {
                log("in 2nd launch")
                Integer.parseInt("a")
            }
        }
    ...
    fun cancel() {
        ...
        scope.launch {
            // 4-й scope отменен, в нем больше ничего не запустить
            println("No chancel")
        }
    ...
    fun handler3() {
        val scope = CoroutineScope(Dispatchers.Default + handler("top"))
        scope.launch(CoroutineName("Cor-1")) {
            launch(handler("child") + CoroutineName("Cor-1.1") + SupervisorJob(coroutineContext.job)) {
                Integer.parseInt("a")
            }
        }
