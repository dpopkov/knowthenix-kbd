...
import io.dpopkov.knowthenixkbd.mappers.v2.exceptions.UnknownRequestClass
...
fun KnthContext.fromTransport(request: IRequest): Unit = when(request) {
    ...
    is TranslationSearchRequest -> fromSearch(request)
        ...
        throw UnknownRequestClass(request::class)
    }
}
...
private fun String?.toQuestionId(): KnthQuestionId = this?.let { KnthQuestionId(it) } ?: KnthQuestionId.NONE
...
private fun TranslationCreateObject.toInternal(): KnthTranslation = KnthTranslation(
    ...
    questionId = this.questionId.toQuestionId(),
...
private fun TranslationDebug?.transportToStubCase(): KnthTranslationStubs = when(this?.stub) {
    ...
    TranslationRequestDebugStubs.BAD_QUESTION_ID -> KnthTranslationStubs.BAD_QUESTION_ID
