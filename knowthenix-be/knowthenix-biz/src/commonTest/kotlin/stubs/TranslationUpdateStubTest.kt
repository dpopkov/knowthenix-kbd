...
class TranslationUpdateStubTest {
    private val processor = KnthTranslationProcessor()
    private val idToUpdate = KnthTranslationId("id to update")
    ...
    private val ownerId = KnthUserId("non-stub owner id")
    ...
    fun testUpdate() = runTest {
        ...
        assertEquals(idToUpdate, ctx.translationResponse.id)
        ...
        assertEquals(ownerId, ctx.translationResponse.ownerId)
    ...
    fun testNoCase() = runTest {
        val ctx = KnthContext(
            ...
            state = KnthState.NONE,
            workMode = KnthWorkMode.STUB,
            stubCase = KnthStubs.BAD_SEARCH_STRING,
            translationRequest = KnthTranslation(
                id = idToUpdate,
            )
        )
        ...
        assertEquals("Wrong stub case is requested: BAD_SEARCH_STRING", err?.message)
    }
}
